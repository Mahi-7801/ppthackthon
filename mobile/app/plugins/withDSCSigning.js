const { withDangerousMod, withAndroidManifest, withMainApplication, withXcodeProject, withInfoPlist } = require('@expo/config-plugins');
const fs = require('fs');
const path = require('path');

// ══════════════════════════════════════════════════════════════
// iOS: CryptoTokenKit Swift Module Injection
// ══════════════════════════════════════════════════════════════

/**
 * Copies DSCSigningModule.swift and DSCSigningModule.m into the iOS Xcode project.
 * CryptoTokenKit gives direct smart card access on iPhone 15/16 USB-C (iOS 16+)
 */
function withDSCSigningIOS(config) {
  return withDangerousMod(config, [
    'ios',
    async (config) => {
      const projectRoot = config.modRequest.platformProjectRoot;
      const pluginDir = path.join(projectRoot, '..', '..', 'plugins', 'native', 'ios');

      // Target directory inside the iOS Xcode project
      const appName = config.modRequest.projectName || 'SecureSign';
      const targetDir = path.join(projectRoot, appName);

      fs.mkdirSync(targetDir, { recursive: true });

      const iosFiles = ['DSCSigningModule.swift', 'DSCSigningModule.m'];
      for (const file of iosFiles) {
        const src = path.join(pluginDir, file);
        const dest = path.join(targetDir, file);
        if (fs.existsSync(src)) {
          fs.copyFileSync(src, dest);
          console.log(`[withDSCSigning] Copied iOS file: ${file}`);
        } else {
          console.warn(`[withDSCSigning] iOS file not found: ${src}`);
        }
      }

      // Create bridging header so Swift can access React Native Obj-C headers
      const bridgingHeaderPath = path.join(targetDir, `${appName}-Bridging-Header.h`);
      const bridgingHeaderContent = `//
// SecureSign Bridging Header — CryptoTokenKit DSC Module
// Auto-generated by withDSCSigning Expo Config Plugin
//
...
`;
      if (!fs.existsSync(bridgingHeaderPath)) {
        ...
        console.log('[withDSCSigning] Created bridging header');
      }
...
/**
 * Adds CryptoTokenKit entitlement & required Info.plist permissions for iOS
 */
function withDSCSigningInfoPlist(config) {
  return withInfoPlist(config, (config) => {
    // USB Device access description (required for App Store)
    config.modResults['NFCReaderUsageDescription'] =
      'SecureSign uses NFC to communicate with your DSC smart card token for digital signing.';

    config.modResults['NSCameraUsageDescription'] =
      'SecureSign may use the camera to scan QR codes for document verification.';

    // CryptoTokenKit smart card access
    config.modResults['com.apple.security.smartcard'] = true;
...
module.exports = function (config) {
  // Android: Native Kotlin CCID USB Driver
  config = withDSCSigning(config);
  config = withDSCSigningMainApplication(config);
  config = withDSCSigningManifest(config);

  // iOS: CryptoTokenKit Swift Smart Card Module (iPhone 15/16 USB-C + BLE/NFC tokens)
  config = withDSCSigningIOS(config);
  config = withDSCSigningInfoPlist(config);
